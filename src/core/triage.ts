...
import type { FeedbackItem, TicketCategory, TicketSeverity, TriagedTicket } from "./types.js";

const REAL_BUG_CATEGORIES: TicketCategory[] = ["crash", "ui", "perf", "copy"];
...
export async function triageFeedback(opts: TriageOptions): Promise<TriagedTicket[]> {
  const log = opts.log ?? (() => {});
  if (opts.items.length === 0) throw new Error("triageFeedback: no feedback items");
...
async function classifyAndCluster(opts: TriageOptions): Promise<Cluster[]> {
  ...
  if (!text || text.type !== "text") throw new Error("Classify pass returned no text");
  const parsed = JSON.parse(text.text) as { clusters: Cluster[] };
...
async function writeTicket(
  ...
  const first = items[0];
  if (!first) throw new Error(`Cluster for ${ticketId} has no items`);
  ...
  const text = res.content.find((b) => b.type === "text");
  if (!text || text.type !== "text") throw new Error(`Ticket pass returned no text for ${ticketId}`);
  const t = JSON.parse(text.text) as {
    title: string;
    whatHappened: string;
    likelyRepro: string[];
    affectedArea: string;
...
function clusterHint(cluster: Cluster): string {
