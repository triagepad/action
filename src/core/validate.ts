...
export function verifyCandidate(
  repo: RepoAccess,
  candidate: Pick<LocalisationCandidate, "file" | "symbol" | "line">,
    ...
    if (token) {
      ...
      if (
        ...
        const nearest = matches.reduce((a, b) =>
          Math.abs(a.line - candidate.line!) < Math.abs(b.line - candidate.line!) ? a : b,
        );
        return {
          status: "verified",
 ...
 */
function symbolToken(symbol: string): string | null {
