...
import { render, type MethodologyBundle } from "./methodology.js";
...
function buildTools(repo: RepoAccess, m: MethodologyBundle): Anthropic.Tool[] {
...
export type LocalisationRoute =
  | { kind: "fixed"; ticket: FixedTicket }
  | { kind: "processed"; fix: FixPrompt };
 ...
 */
export function routeLocalisation(
  bug: TriagedTicket,
  parsed: LocalisationAgentResult,
  repo: RepoAccess,
): LocalisationRoute {
  ...
  const candidates: LocalisationCandidate[] = parsed.candidates.map((c) => ({
    ...c,
...
export async function localiseTickets(opts: LocaliseOptions): Promise<FixesOutput> {
  const log = opts.log ?? (() => {});
  const output: FixesOutput = {
    version: 1,
...
async function localiseBug(
  opts: LocaliseOptions,
  bug: TriagedTicket,
  fileTree: string[],
): Promise<LocalisationAgentResult> {
  ...
  for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
    ...
    for (const block of res.content) {
      ...
      results.push({
        ...
        content: runTool(opts.repo, block.name, block.input as Record<string, unknown>),
      });
    }
  ...
  if (!text || text.type !== "text") throw new Error(`No final answer for ${bug.id}`);
  return JSON.parse(text.text) as LocalisationAgentResult;
}

function runTool(repo: RepoAccess, name: string, input: Record<string, unknown>): string {
  try {
    if (name === "grep") {
    ...
    if (name === "git_log" && repo.history) {
      ...
      if (commits.length === 0) return "No history available for these paths.";
