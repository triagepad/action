...
export type PlannedAction =
  | { kind: "open-pr"; ticket: TriagedTicket; fix: FixPrompt; autoMerge: boolean }
  | { kind: "plan-issue"; ticket: TriagedTicket; fix: FixPrompt }
  | { kind: "decision-issue"; ticket: TriagedTicket; fix: FixPrompt }
  | { kind: "feature-issue"; ticket: TriagedTicket; reason: string }
  | { kind: "report-fixed"; ticket: TriagedTicket; reason: string; resolvedBy: string | null; evidenceSource: string | null }
  | { kind: "report-skip"; ticket: TriagedTicket; reason: string }
  | { kind: "report-not-eligible"; ticket: TriagedTicket; fix: FixPrompt; reason: string };

const CONFIDENCE_RANK: Record<Confidence, number> = { low: 1, medium: 2, high: 3 };

export function routeForCi(
...
export function feedbackMarkers(ticket: TriagedTicket): string {
...
export function ticketBody(ticket: TriagedTicket, fix: FixPrompt): string {
  return [
    ...
    `|---|------|------|------------|--------|--------------|`,
    ...fix.candidates.map(
      (c, i) => `| ${i + 1} | \`${c.file}\` | ${c.line ?? "–"} | ${c.confidence.toFixed(2)} | ${c.signal} | ${c.verification.status} |`,
    ),
    "",
    `### Explanation`,
    fix.explanation,
...
export function featureIssueBody(
  ...
  opts?: { testerText?: string | null; screenshotRef?: string | null },
): string {
  return [
...
export function decisionIssueBody(ticket: TriagedTicket, fix: FixPrompt): string {
  return [
