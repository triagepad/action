...
import { executeActions, type ExecutionResult, type ExecutorConfig } from "./github.js";
import { routeForCi, type Confidence, type FeatureRequestMode, type FixMode } from "./route.js";

const ENGINE_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");

function input(name: string, fallback?: string): string {
  const value = process.env[`INPUT_${name.toUpperCase()}`] ?? fallback;
  if (value === undefined) throw new Error(`Missing required action input: ${name}`);
  return value;
}
...
async function main(): Promise<void> {
  ...
  if (dryRun) {
    ...
    const tickets = readJson<TriagedTicket[]>(join(ENGINE_ROOT, "tests/fixtures/action-dryrun/tickets.json"));
    const fixes = readJson<FixesOutput>(join(ENGINE_ROOT, "tests/fixtures/action-dryrun/fixes.json"));
    const actions = routeForCi(tickets, fixes, {
      mode: (optionalInput("mode") ?? "pr") as FixMode,
      minConfidence: (optionalInput("min_confidence") ?? "high") as Confidence,
      featureRequests: (optionalInput("feature_requests") ?? "issue") as FeatureRequestMode,
    });
  ...
  const mode = (optionalInput("mode") ?? brainConfig.defaultMode) as FixMode;
  const minConfidence = (optionalInput("min_confidence") ?? brainConfig.minConfidence) as Confidence;
  ...
  if (inference === "byo") process.env.ANTHROPIC_API_KEY = input("anthropic_api_key");
  ...
  const readAttachment = (path: string): Buffer | null => {
  ...
  const tickets = await triageFeedback({
    client,
    models,
    methodology: methodology as unknown as MethodologyBundle,
    items: newItems,
    appContext,
    firstTicketNumber,
    readAttachment,
  ...
  const featureRequests = (optionalInput("feature_requests") ?? "issue") as FeatureRequestMode;
  const actions = routeForCi(tickets, fixes, { mode, minConfidence, featureRequests });
  const feedbackLookup = new Map(
    newItems.map((i) => [i.id, { text: i.text, screenshotPath: i.screenshotPath }]),
  );
  const server = process.env.GITHUB_SERVER_URL ?? "https://github.com";
  const runArtifactUrl =
  ...
  await brain.postResults({
    ...
    tickets,
    fixes,
    executions: results.map((r: ExecutionResult) => ({ ticketId: r.ticketId, outcome: r.outcome, url: r.url })),
  });
...
function ascPullConfig(outRoot: string, appContext: string, models: Config["models"]): Config {
  ...
  return {
    ...
    server: {} as Config["server"], // unused in CI
  };
}
...
function detectDefaultBranch(repoDir: string): string {
  try {
    ...
    if (event && existsSync(event)) {
      const payload = JSON.parse(readFileSync(event, "utf8")) as { repository?: { default_branch?: string } };
      if (payload.repository?.default_branch) return payload.repository.default_branch;
    }
  } catch {
    /* fall through */
  }
...
function appendSummary(text: string): void {
  ...
  if (path) writeFileSync(path, text, { flag: "a" });
  console.log(text);
}
