...
export async function runPull(config) {
    ...
    for (const dir of [raw, screenshotsDir, crashlogsDir])
...
async function normaliseScreenshots(client, res, dir, config) {
    const items = [];
    for (const sub of res.data) {
        const attrs = sub.attributes;
        const images = attrs?.screenshots ?? [];
        ...
        for (const [n, image] of images.entries()) {
            try {
                ...
                console.warn(`  ! screenshot download failed for ${sub.id}: ${err.message}`);
            }
        }
        items.push({
            id: sub.id,
...
async function normaliseCrashes(client, res, dir, config) {
    const items = [];
    for (const sub of res.data) {
        const attrs = sub.attributes;
        let crashLogPath = null;
        try {
            const logRes = (await client.getCrashLog(sub.id));
            ...
            console.warn(`  ! crash log fetch failed for ${sub.id}: ${err.message}`);
        }
        items.push({
            id: sub.id,
            type: "crash",
...
function printSummary(items, config) {
