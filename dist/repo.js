...
export function localRepoAccess(repoRoot) {
    const safePath = (path) => {
        const full = join(repoRoot, path);
        if (relative(repoRoot, full).startsWith(".."))
            throw new Error(`Path escapes repo: ${path}`);
        return full;
    };
    return {
        listFiles() {
        ...
        grep(pattern, isRegex, maxMatches = 60) {
            let regex;
            try {
        ...
        isAncestor(ancestor, descendant) {
            if (!/^[0-9a-f]{4,40}$/i.test(ancestor) || !/^[0-9a-f]{4,40}$/i.test(descendant))
                return false;
            try {
