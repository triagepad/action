...
function isFixPrExecution(e) {
    if (e.url && /\/pull\/\d+/.test(e.url))
        return true;
    return /\bpr opened\b/i.test(e.outcome);
}
function isMergedExecution(e) {
...
export function deriveTicketStatus(i) {
    if (i.actions.dismissedAt)
        return "dropped";
    ...
    if (i.skipped)
        return i.ticket.category === "feature-request" ? "feature-request" : "new";
    if (i.fixed)
        return "fixed-stale";
    // Only genuine fix PRs count as pr-open/merged (real /pull/ execution), never issues.
    if (i.executions.some(isMergedExecution))
        return "merged";
    if (i.executions.some(isFixPrExecution))
        return "pr-open";
    // Optimistic: a dispatched pr/autofix shows as pr-open until the run reports back.
    if (i.actions.dispatchedMode === "pr" || i.actions.dispatchedMode === "autofix")
        return "pr-open";
    if (i.fix?.needsDecision.required && i.actions.decisionOption === null)
 ...
 */
export function dispatchEligibility(fix, mode, decided) {
    if (mode === "plan")
        return { ok: true };
    if (!fix)
        return { ok: false, reason: "ticket is not localised — nothing to dispatch" };
    if (!fix.validationPassed)
        return { ok: false, reason: "the localisation did not pass validation" };
