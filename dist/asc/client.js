...
export function ascClientFromCreds(creds) {
...
export class AscClient {
    ...
    async listScreenshotFeedback(limit) {
        return (await this.get(`/v1/apps/${this.config.asc.appId}/betaFeedbackScreenshotSubmissions`, {
            sort: "-createdDate",
            limit: String(Math.min(limit, 200)),
    ...
    async listCrashFeedback(limit) {
        return (await this.get(`/v1/apps/${this.config.asc.appId}/betaFeedbackCrashSubmissions`, {
            sort: "-createdDate",
            limit: String(Math.min(limit, 200)),
            include: "build,tester",
            "fields[builds]": "version,preReleaseVersion",
    ...
    async getCrashLog(submissionId) {
    ...
    async post(path, body) {
        ...
        if (!res.ok)
            throw new AscError(`ASC ${res.status} on POST ${path}: ${text.slice(0, 2000)}`, res.status);
        return text ? JSON.parse(text) : {};
    }
    /** The app's existing webhooks — used to make registration idempotent. */
    async listWebhooks(appId) {
        const res = (await this.get(`/v1/apps/${appId}/webhooks`, { limit: "200" }));
        return res.data.map((w) => ({
            id: w.id,
            url: w.attributes?.url ?? null,
            enabled: w.attributes?.enabled ?? false,
        }));
    }
